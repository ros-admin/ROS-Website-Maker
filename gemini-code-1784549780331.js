 ...
 * মেম্বার MANAGEMENT সেকশনের সম্পূর্ণ ডিজাইন, টেবিল রেন্ডারিং, ফিল্টার, এডমিন মেম্বার এড এবং এডিটিং লজিক
 */
  ...
  container.innerHTML = `
    <section id="member_management-section" class="space-y-6 animate-fadeIn">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onclick="openAddMemberModal()" class="px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold text-xs rounded-xl shadow-lg flex items-center gap-2 cursor-pointer transition-all">
    ...
    <div id="memberPopupModal" class="hidden fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div class="bg-slate-900 border-2 border-[#00b4d8]/40 rounded-2xl max-w-3xl w-full max-h-[92vh] overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div id="modalPrintArea" class="p-6 space-y-6 flex-1 text-center">
          <div class="flex flex-col sm:flex-row items-center gap-6 bg-slate-950/40 p-4 border border-slate-800 rounded-xl text-left">
            ...
            <div class="space-y-2 w-full">
              <div class="flex justify-between items-center">
                ...
                <button onclick="openEditMemberModal()" class="px-3 py-1 bg-amber-600/20 hover:bg-amber-600 text-amber-400 hover:text-white border border-amber-500/30 rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer">
          ...
          <!-- Admin Note / Status Reason Display -->
          <div class="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 text-left">
            <span class="text-amber-400 font-bold block mb-1 text-xs"><i class="fa-solid fa-note-sticky mr-1"></i> Admin Note / Status Reason:</span>
            <p id="modal-status-reason" class="text-slate-200 text-xs font-mono bg-slate-950/60 p-2 rounded border border-slate-800/80 break-words">N/A</p>
          </div>
    ...
    <!-- ২. এডমিন কর্তৃক সরাসরি নতুন মেম্বার এড করার মডাল (Updated Fields) -->
    <div id="addMemberModal" class="hidden fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div class="bg-slate-900 border-2 border-[#00b4d8]/40 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl p-6">
        <div class="flex justify-between items-center border-b border-slate-800 pb-3 mb-4">
          <h3 class="text-md font-bold text-[#00b4d8] flex items-center gap-2"><i class="fa-solid fa-user-plus"></i> Add New Member (Admin Direct)</h3>
          <button onclick="closeAddMemberModal()" class="text-slate-400 hover:text-white"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
        <form id="adminAddMemberForm" onsubmit="submitAdminAddMember(event)" class="space-y-4 text-xs">
          <input type="hidden" id="add_photoUrl" value="">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-slate-400 mb-1 font-bold">Name (Bangla)</label>
              <input type="text" id="add_banglaName" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1 font-bold">Name (English) <span class="text-rose-500">*</span></label>
              <input type="text" id="add_englishName" required class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Father's Name</label>
              <input type="text" id="add_fatherName" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Mother's Name</label>
              <input type="text" id="add_motherName" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1 font-bold">Mobile Number <span class="text-rose-500">*</span></label>
              <input type="text" id="add_mobile" required class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Email Address</label>
              <input type="email" id="add_email" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Blood Group</label>
              <select id="add_bloodGroup" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            ...
            <div>
              ...
              <select id="add_gender" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
                <option value="Male">Male</option>
                <option value="Female">Female</option>
                <option value="Other">Other</option>
              </select>
            ...
            <div>
              <label class="block text-slate-400 mb-1">DOB</label>
              <input type="date" id="add_dob" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">Present Address</label>
              <input type="text" id="add_presentAddress" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">Permanent Address</label>
              <input type="text" id="add_permanentAddress" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Education</label>
              <input type="text" id="add_education" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Academic Year</label>
              <input type="text" id="add_academicYear" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Profession</label>
              <input type="text" id="add_profession" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Institute</label>
              <input type="text" id="add_institution" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Whatsapp Number</label>
              <input type="text" id="add_whatsapp" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">FB ID Link</label>
              <input type="text" id="add_fbLink" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">NID/Birth Certificate Number</label>
              <input type="text" id="add_nid" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">Photo (max 1 MB)</label>
              <input type="file" id="add_photoFile" accept="image/*" onchange="uploadImageToCloudinary(this, 'add')" class="w-full text-slate-400 text-xs file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-[#00b4d8]/10 file:text-[#00b4d8] hover:file:bg-[#00b4d8]/20 cursor-pointer">
              <span id="add_photo_status" class="text-[10px] text-emerald-400 mt-1 block"></span>
            </div>
          </div>

          <div class="flex justify-end gap-2 pt-3 border-t border-slate-800">
            <button type="button" onclick="closeAddMemberModal()" class="px-4 py-2 bg-slate-800 text-slate-300 rounded-lg">Cancel</button>
            <button type="submit" class="px-5 py-2 bg-[#00b4d8] text-slate-950 font-bold rounded-lg hover:bg-cyan-400">Save Member</button>
          </div>
        </form>
      </div>
    </div>

    <!-- ৩. মেম্বারের তথ্য এডিটিং মডাল (Updated Fields) -->
    <div id="editMemberModal" class="hidden fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div class="bg-slate-900 border-2 border-amber-500/40 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl p-6">
        <div class="flex justify-between items-center border-b border-slate-800 pb-3 mb-4">
          <h3 class="text-md font-bold text-amber-400 flex items-center gap-2"><i class="fa-solid fa-user-pen"></i> Edit Member Details</h3>
          <button onclick="closeEditMemberModal()" class="text-slate-400 hover:text-white"><i class="fa-solid fa-xmark text-lg"></i></button>
        </div>
        <form id="adminEditMemberForm" onsubmit="submitAdminEditMember(event)" class="space-y-4 text-xs">
          <input type="hidden" id="edit_memberId">
          <input type="hidden" id="edit_photoUrl" value="">
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label class="block text-slate-400 mb-1 font-bold">Name (Bangla)</label>
              <input type="text" id="edit_banglaName" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1 font-bold">Name (English)</label>
              <input type="text" id="edit_englishName" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Father's Name</label>
              <input type="text" id="edit_fatherName" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Mother's Name</label>
              <input type="text" id="edit_motherName" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1 font-bold">Mobile Number</label>
              <input type="text" id="edit_mobile" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1 font-bold">Email Address</label>
              <input type="email" id="edit_email" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Blood Group</label>
              <select id="edit_bloodGroup" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
                <option value="">Select</option>
                <option value="A+">A+</option><option value="A-">A-</option>
                <option value="B+">B+</option><option value="B-">B-</option>
                <option value="O+">O+</option><option value="O-">O-</option>
            ...
            <div>
              <label class="block text-slate-400 mb-1">Gender</label>
              <select id="edit_gender" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
                <option value="Male">Male</option>
                <option value="Female">Female</option>
                <option value="Other">Other</option>
              </select>
            ...
            <div>
              <label class="block text-slate-400 mb-1">DOB</label>
              <input type="text" id="edit_dob" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">Present Address</label>
              <input type="text" id="edit_presentAddress" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">Permanent Address</label>
              <input type="text" id="edit_permanentAddress" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Education</label>
              <input type="text" id="edit_education" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Academic Year</label>
              <input type="text" id="edit_academicYear" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Profession</label>
              <input type="text" id="edit_profession" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Institute</label>
              <input type="text" id="edit_institution" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">Whatsapp Number</label>
              <input type="text" id="edit_whatsapp" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div>
              <label class="block text-slate-400 mb-1">FB ID Link</label>
              <input type="text" id="edit_fbLink" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">NID/Birth Certificate Number</label>
              <input type="text" id="edit_nid" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white">
            </div>
            <div class="sm:col-span-2">
              <label class="block text-slate-400 mb-1">Photo (max 1 MB - Upload to Change)</label>
              <input type="file" id="edit_photoFile" accept="image/*" onchange="uploadImageToCloudinary(this, 'edit')" class="w-full text-slate-400 text-xs file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-amber-500/10 file:text-amber-400 hover:file:bg-amber-500/20 cursor-pointer">
              <span id="edit_photo_status" class="text-[10px] text-amber-400 mt-1 block"></span>
            </div>
          </div>

          <div class="flex justify-end gap-2 pt-3 border-t border-slate-800">
            <button type="button" onclick="closeEditMemberModal()" class="px-4 py-2 bg-slate-800 text-slate-300 rounded-lg">Cancel</button>
            <button type="submit" class="px-5 py-2 bg-amber-500 text-slate-950 font-bold rounded-lg hover:bg-amber-400">Update Info</button>
          </div>
        </form>
      </div>
    </div>
...
async function uploadImageToCloudinary(fileInput, mode) {
  const file = fileInput.files[0];
  if (!file) return;

  if (file.size > 1024 * 1024) {
    alert("ফাইল সাইজ সর্বোচ্চ ১ Megabyte (1 MB) হতে পারবে!");
    fileInput.value = "";
    return;
  }

  const statusEl = document.getElementById(`${mode}_photo_status`);
  if (statusEl) statusEl.innerText = "ছবি ক্লাউডিনারিতে আপলোড হচ্ছে...";
  ...
  try {
    ...
    if (data.secure_url) {
      document.getElementById(`${mode}_photoUrl`).value = data.secure_url;
      if (statusEl) statusEl.innerText = "✓ ছবি সফলভাবে আপলোড হয়েছে!";
    } else {
      if (statusEl) statusEl.innerText = "✖ আপলোড ব্যর্থ হয়েছে।";
      alert("ছবি আপলোডে সমস্যা হয়েছে। Upload Preset ও Cloud Name সঠিক আছে কিনা পরীক্ষা করুন।");
    }
  } catch (err) {
    console.error(err);
    if (statusEl) statusEl.innerText = "✖ আপলোড ব্যর্থ হয়েছে।";
    alert("ক্লাউডিনারিতে ছবি আপলোড করার সময়ে নেটওয়ার্ক ত্রুটি ঘটেছে।");
  }
}
...
// স্ট্যাটাস পরিবর্তনের সময় কারণ চাওয়া এবং ফিক্সড আপডেটিং
async function liveStatusUpdateDirect(memberId, newStatus) {
  if(!newStatus) return;
  
  let reason = "Ok";
  if(newStatus !== "active") {
    reason = prompt(`স্ট্যাটাস ${newStatus.toUpperCase()} করার কারণ লিখুন:`);
    if(reason === null || reason.trim() === "") {
      alert("কারণ উল্লেখ না করায় স্ট্যাটাস পরিবর্তন করা হয়নি।");
      filterAndRenderMembersTable();
      return;
    }
  }

  if(typeof showLoader === 'function') showLoader(true, "গুগল শীটে স্ট্যাটাস ও কারণ আপডেট হচ্ছে...");
  try {
    const response = await fetch(WEB_APP_URL, {
      method: 'POST', mode: 'cors', headers: { 'Content-Type': 'text/plain' },
      body: JSON.stringify({ action: "updateStatus", memberId: memberId, status: newStatus, reason: reason })
    });
    const res = await response.json();
    if(typeof showLoader === 'function') showLoader(false);
    if(res.success) {
      alert("সদস্যের স্ট্যাটাস ও কারণ আপডেট করা হয়েছে!");
      const idx = window.allUsersData.findIndex(u => u.memberId === memberId);
      if(idx !== -1) {
        window.allUsersData[idx].status = newStatus;
        window.allUsersData[idx].statusReason = reason;
      }
      filterAndRenderMembersTable();
...
function openMemberModal(memberId) {
  ...
  // Status Reason Box Populate
  document.getElementById('modal-status-reason').innerText = user.statusReason || 'N/A / None';
...
// এডমিন কর্তৃক নতুন সদস্য যোগ করার লজিক
...
function closeAddMemberModal() { 
  ...
  document.getElementById('add_photoUrl').value = '';
  document.getElementById('add_photo_status').innerText = '';
}

async function submitAdminAddMember(e) {
  e.preventDefault();
  const payload = {
    action: "adminAddMember",
    banglaName: document.getElementById('add_banglaName').value,
    englishName: document.getElementById('add_englishName').value,
    fatherName: document.getElementById('add_fatherName').value,
    motherName: document.getElementById('add_motherName').value,
    mobileNumber: document.getElementById('add_mobile').value,
    email: document.getElementById('add_email').value,
    bloodGroup: document.getElementById('add_bloodGroup').value,
    gender: document.getElementById('add_gender').value,
    dob: document.getElementById('add_dob').value,
    presentAddress: document.getElementById('add_presentAddress').value,
    permanentAddress: document.getElementById('add_permanentAddress').value,
    education: document.getElementById('add_education').value,
    academicYear: document.getElementById('add_academicYear').value,
    profession: document.getElementById('add_profession').value,
    institution: document.getElementById('add_institution').value,
    whatsappNumber: document.getElementById('add_whatsapp').value,
    facebookLink: document.getElementById('add_fbLink').value,
    nidOrBrn: document.getElementById('add_nid').value,
    photoUrl: document.getElementById('add_photoUrl').value
  };

  if(typeof showLoader === 'function') showLoader(true, "নতুন মেম্বার এড করা হচ্ছে...");
  try {
    ...
    if(typeof showLoader === 'function') showLoader(false);
    if(result.success) {
      alert(`মেম্বার সফলভাবে তৈরি হয়েছে! ID: ${result.memberId}`);
      closeAddMemberModal();
      location.reload(); 
    } else { alert(result.error || "ত্রুটি ঘটেছে।"); }
  } catch(err) {
    if(typeof showLoader === 'function') showLoader(false);
...
// এডমিন কর্তৃক সদস্যের তথ্য সম্পাদন (Edit) করার লজিক
function openEditMemberModal() {
  if(!window.activePopupUser) return;
  const u = window.activePopupUser;
  document.getElementById('edit_memberId').value = u.memberId;
  document.getElementById('edit_banglaName').value = u.banglaName || '';
  document.getElementById('edit_englishName').value = u.englishName || '';
  document.getElementById('edit_fatherName').value = u.fatherName || '';
  document.getElementById('edit_motherName').value = u.motherName || '';
  document.getElementById('edit_mobile').value = u.mobile || '';
  document.getElementById('edit_email').value = u.email || '';
  document.getElementById('edit_bloodGroup').value = u.bloodGroup || '';
  document.getElementById('edit_gender').value = u.gender || 'Male';
  document.getElementById('edit_dob').value = u.dob || '';
  document.getElementById('edit_presentAddress').value = u.presentAddress || '';
  document.getElementById('edit_permanentAddress').value = u.permanentAddress || '';
  document.getElementById('edit_education').value = u.education || '';
  document.getElementById('edit_academicYear').value = u.academicYear || '';
  document.getElementById('edit_profession').value = u.profession || '';
  document.getElementById('edit_institution').value = u.institution || '';
  document.getElementById('edit_whatsapp').value = u.whatsappNumber || '';
  document.getElementById('edit_fbLink').value = u.facebookLink || '';
  document.getElementById('edit_nid').value = u.nidOrBrn || '';
  document.getElementById('edit_photoUrl').value = u.photoUrl || '';
...
async function submitAdminEditMember(e) {
  e.preventDefault();
  const payload = {
  ...
  if(typeof showLoader === 'function') showLoader(true, "মেম্বারের তথ্য আপডেট হচ্ছে...");
  try {
    const res = await fetch(WEB_APP_URL, {
      method: 'POST', mode: 'cors', headers: {'Content-Type': 'text/plain'},
      body: JSON.stringify(payload)
...
function exportToPDF() {
  ...
  const bodyRows = window.currentFilteredList.map((u, i) => ["", i + 1, u.memberId || '', u.englishName || '', u.mobile || '', u.email || '', u.bloodGroup || '', (u.status || '').toUpperCase(), u.statusReason || '']);
  doc.autoTable({
    startY: 25,
    head: [['[ ]', 'SL', 'Reg ID', 'Name', 'Mobile', 'Email', 'Blood', 'Status', 'Reason']],
    body: bodyRows,
    theme: 'grid'
  });
    ...
    const generateQrDataUrlWithLogo = async (text, logoSrc) => {
      return new Promise((resolve) => {
        ...
        qrImg.onload = () => {
          ...
          logo.onerror = () => resolve(canvas.toDataURL('image/png', 1.0));
        };
        qrImg.onerror = () => resolve(qrApiUrl);
      });
    };
      ...
      <head>
        ...
        <style>
          ...
          * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          body { margin: 0; padding: 0; font-family: Arial, sans-serif; font-size: 8.5pt; line-height: 1.2; color: #000; background: #fff; }
        </style>
      </head>
      <body>
...
window.submitAdminAddMember = submitAdminAddMember;
window.openEditMemberModal = openEditMemberModal;
window.closeEditMemberModal = closeEditMemberModal;
window.submitAdminEditMember = submitAdminEditMember;
