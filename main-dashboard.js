/**
 * ==========================================
 * ROS Admin Dashboard - Main Dashboard Module
 * ==========================================
 */
...
// মূল ড্যাশবোর্ডের লেআউট ডিজাইন ইনজেকশন
function initMainDashboardHTML() {
  const container = document.getElementById('main_dashboard-section');
  if (!container) return;
...
// চার্ট ক্যালকুলেশন ও রেন্ডারিং ফাংশন
function renderDashboardCharts(users) {
  ...
  // ১. লাইন চার্ট
  const lineCtx = document.getElementById('chart-last-7-days');
  if (lineCtx) {
    ...
    lineChartInstance = new Chart(lineCtx.getContext('2d'), {
      ...
      data: {
        ...
        datasets: [{
          label: 'রেজিস্ট্রেশন',
          data: last7Days.map(d => dateCounts[d]),
          borderColor: '#ec4899',
  ...
  // ২. পাই চার্ট
  const pieCtx = document.getElementById('chart-gender-wise');
  if (pieCtx) {
    if (pieChartInstance) pieChartInstance.destroy();
    pieChartInstance = new Chart(pieCtx.getContext('2d'), {
      type: 'pie',
      data: {
        labels: ['ছেলে (Male)', 'মেয়ে (Female)', 'অন্যান্য'],
        datasets: [{
          data: [genderCounts['Male'], genderCounts['Female'], genderCounts['Other']],
          backgroundColor: ['#00b4d8', '#ec4899', '#f59e0b'],
          borderWidth: 0
        }]
      },
      options: {
  ...
  // ৩. বার চার্ট
  const barCtx = document.getElementById('chart-year-wise');
  if (barCtx) {
    ...
    barChartInstance = new Chart(barCtx.getContext('2d'), {
      ...
      data: {
        labels: last5Years,
        datasets: [{
          data: last5Years.map(y => yearCounts[y]),
          backgroundColor: ['#3b82f6', '#10b981', '#00b4d8', '#f59e0b', '#8b5cf6'],
          borderRadius: 8,
          barThickness: 28
        }]
      },
